...
import { TrashbinItem } from '@/api/server/files/trashbin';
...
export const rawDataToTrashbinItem = ({ attributes }: FractalResponseData): TrashbinItem => ({
    trashRoot: attributes.trash_root,
    trashName: attributes.trash_name,
    displayName: attributes.display_name,
    originalRoot: attributes.original_root ?? null,
    originalName: attributes.original_name ?? null,
    isDirectory: attributes.is_directory,
    isFile: attributes.is_file,
    size: Number(attributes.size),
    mimetype: attributes.mimetype,
    createdAt: new Date(attributes.created_at),
    modifiedAt: new Date(attributes.modified_at),
    hasMetadata: attributes.has_metadata,
});
