...
export default ({ server, count }: { server: Server; count: number }) => {
    ...
    return (
        <>
            <CardAnimator
                className='backdrop boxBorder rounded-box overflow-hidden'
                css={'background-color: rgb(var(--gray700-default));'}
                style={{ animationDelay: `${count * 100}ms` }}
            >
                <Link
                    to={`/server/${server.id}`}
                    className={'flex justify-between bg-center bg-cover bg-no-repeat relative px-6 py-5 z-10'}
                    css={`
                ...
                >
                    <div
                        ...
                        css={
                            'background-image:linear-gradient(-90deg, rgb(var(--gray700-default)) 0%, color-mix(in srgb, rgb(var(--gray700-default)) 65%, transparent) 100%);'
                        }
                    />
                    <div>
                        ...
                        <p className='mt-1 text-gray-200'>{server.description}</p>
                    </div>
                ...
                {/* <Link
                    ...
                    className={'block bg-center bg-cover bg-no-repeat relative px-6 pt-5 z-10'}
                    css={`
                        background-image: url(${server.eggImage ? server.eggImage : '/arix/minecraft-banner.png'});
                    `}
                >
                    <div
                        className={'z-[-1] absolute inset-0'}
                        css={
                            'background-image:linear-gradient(0deg, rgb(var(--gray700-default)) 0%, color-mix(in srgb, rgb(var(--gray700-default)) 65%, transparent) 100%);'
                        }
                    />
                    <div className='flex items-center justify-between pb-5'>
                        <p className='text-lg font-semibold text-gray-50'>{server.name}</p>
                        <Badge color={(stats?.status === 'offline' || stats?.status === 'stopping') ? 'danger' : stats?.status === 'running' ? 'success' : 'warning'}>
                            {stats?.status === 'offline'
                                ? t('offline', { ns: 'arix/utilities' })
                                : stats?.status === 'running'
                                ? t('online', { ns: 'arix/utilities' })
